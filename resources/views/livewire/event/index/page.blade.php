<div class="space-y-6">
    ...
    <flux:table :paginate="$this->events">
        ...
        <flux:columns>
            ...
            <flux:column sortable :sorted="$sortBy === 'image'" :direction="$sortDirection" wire:click="sort('image')">{{ __('event.index.table.header.image') }}</flux:column>
        ...
        <flux:rows>
            @foreach ($this->events as $event)
                <flux:row :key="$event->id">
                    ...
                    <flux:cell>
                        @if($event->image)
                            <flux:icon icon="photo" class="size-6 ml-3" />
                        @else
                        <flux:icon icon="x-mark" class="size-4 ml-3" />
                        @endif
                    </flux:cell>
