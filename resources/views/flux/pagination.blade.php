...
@php
    $simple = ! $paginator instanceof LengthAwarePaginator;
@endphp
                ...
                @else
                    @if(method_exists($paginator,'getCursorName'))
                        <button type="button"
                                ...
                                class="flex justify-center items-center size-6 rounded-[6px] text-zinc-400 dark:text-white hover:bg-zinc-100 dark:hover:bg-white/20 hover:text-zinc-800 dark:hover:text-white"
                        >
                            <flux:icon.chevron-left variant="micro"/>
                        </button>
                    @else
    ...
    <div {{ $attributes->class('pt-3 border-t border-zinc-100 dark:border-zinc-700 flex justify-between items-center max-sm:flex-col max-sm:gap-3 max-sm:items-end') }} data-flux-pagination>
        ...
        @if ($paginator->hasPages())
            <div class="flex items-center bg-white border border-zinc-200 rounded-[8px] p-[1px] dark:bg-white/10 dark:border-white/10">
                ...
                @else
                    <div aria-label="{{ __('pagination.next') }}"
                         class="flex justify-center items-center size-6 rounded-[6px] text-zinc-300 dark:text-zinc-400"
                    >
                        <flux:icon.chevron-right variant="micro"/>
                    </div>
