...
class TransactionInvoicePdf extends BasePdfTemplate
{
    ...
    public function __construct(Transaction $transaction, ?Member $member = null, string $locale = 'de', string $documentNumber = '')
    {
    ...
    public function generateContent()
    {
        ...
        $this->SetFont('helvetica', '', 11);
        $this->MultiCell(0,6, $this->transaction->label, 'LBR','L',0,1);
        $this->SetFont('helvetica', '', 8);
        $this->Cell(0,6, __('Referenz') , 'LTR',1);
        $this->SetFont('helvetica', '', 11);
        $this->MultiCell(0,6, $this->transaction->reference ?? '-', 'LBR','L',0,1);
